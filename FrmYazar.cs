...
namespace Kutuphane
{
    public partial class FrmYazar : Form
    {
        ...
        void LoadData()
        {
            dtGridView.DataSource = db.GetYazar();
            if (dtGridView.RowCount == 1) cmdSil.Enabled = false;
            dtGridView.Columns[0].HeaderText = "Yazar No";
            dtGridView.Columns[1].HeaderText = "Yazar Adı";
        ...
        private void cmdKaydet_Click(object sender, EventArgs e)
        {
            ...
            {
                bool isSuccess = db.AddYazar(txtYazarAdi.Text);
                if (isSuccess)
                {
            ...
            else
            {
                ...
                int yazar_id = (int)row.Cells["yazar_id"].Value;
                bool isSuccess = db.UpdateYazar(yazar_id, txtYazarAdi.Text);
                if (isSuccess)
                {
        ...
        private void cmdSil_Click(object sender, EventArgs e)
        {
            if (dtGridView.SelectedRows.Count == 1)
            {
                var row = dtGridView.SelectedRows[0];
                int yazar_id = (int)row.Cells["yazar_id"].Value;
                bool isSuccess = db.DeleteYazar(yazar_id);
                if (isSuccess)
                {
                    MessageBox.Show("Yazar kaydı silindi.");
                    LoadData();
                }
                else
                {
